        ...
        for line in text.lines() {
            ...
            if let Some(event) = openai_chat::parse_sse_chunk(data) {
                ...
                for tc in event.tool_calls {
                    merge_tool_call_into(&mut tool_calls, tc);
                }
...
/// Merge a streamed tool-call delta into the accumulated list, keyed by id.
///
/// OpenAI streams one tool call across multiple SSE chunks: usually the first
/// chunk carries the `id` and function name, and each subsequent chunk appends
/// more `function.arguments` text. `parse_sse_chunk` already assembles all of
/// that for a *single* incoming chunk (including a locally accumulated
/// arguments string via its internal `index` logic). Here we just need to turn
/// the per-chunk results into stable, completed tool calls keyed by `id`.
///
/// - If a chunk whose `id` is already known arrives and carries arguments,
///   those arguments are *appended* (handles servers that send split
///   `arguments` deltas per chunk while keeping the id on every chunk).
/// - If no id at all is present (some gateways omit id on continuation chunks),
///   the arguments are appended to the most recent unfinished tool call.
fn merge_tool_call_into(all: &mut Vec<ToolCall>, tc: ToolCall) {
    // A chunk with an id represents a tool call; find or create the slot.
    if !tc.id.is_empty() {
        if let Some(existing) = all.iter_mut().find(|t| t.id == tc.id) {
            if existing.name.is_empty() {
                existing.name = tc.name.clone();
            }
            // Some gateways resend the full accumulated arguments on every
            // chunk instead of incremental deltas. If the chunk's text is
            // already a suffix of what we hold, skip to avoid duplication.
            if tc.arguments.is_empty() || !existing.arguments.ends_with(&tc.arguments) {
    ...
    // No id on this chunk: it's a continuation of the last tool call.
    if let Some(last) = all.last_mut() {
        if !tc.name.is_empty() { last.name = tc.name.clone(); }
        if tc.arguments.is_empty() || !last.arguments.ends_with(&tc.arguments) {
